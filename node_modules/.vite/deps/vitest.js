    ...
    function printListItems(list, config2, indentation, depth, refs, printer) {
      ...
      if (list.length) {
        ...
        for (let i3 = 0; i3 < list.length; i3++) {
          ...
          if (i3 in list) {
            result += printer(list[i3], config2, indentationNext, depth, refs);
          }
          if (i3 < list.length - 1) {
            result += `,${config2.spacingInner}`;
          } else if (!config2.min) {
    ...
    (function(global3, factory) {
      ...
      function inspectList(list, options, inspectItem) {
        ...
        for (var i3 = 0; i3 < size; i3 += 1) {
          var last = i3 + 1 === list.length;
          var secondToLast = i3 + 2 === list.length;
          truncated = "".concat(truncator, "(").concat(list.length - i3, ")");
          var value = list[i3];
          ...
          if (!last && !secondToLast && nextLength + peek.length >= originalLength) {
            truncated = "".concat(truncator, "(").concat(list.length - i3 - 1, ")");
            break;
          }
          truncated = "";
        }
      ...
      function inspectTypedArray(array2, options) {
        ...
        for (var i3 = 0; i3 < array2.length; i3++) {
          var string3 = "".concat(options.stylize(truncate(array2[i3], options.truncate), "number")).concat(i3 === array2.length - 1 ? "" : ", ");
          options.truncate -= string3.length;
          if (array2[i3] !== array2.length && options.truncate <= 3) {
            output += "".concat(truncator, "(").concat(array2.length - array2[i3] + 1, ")");
            break;
          }
          output += string3;
        }
      ...
      var isNaN2 = Number.isNaN || function(i3) {
        return i3 !== i3;
      };
...
var require_type_detect = __commonJS({
  "node_modules/type-detect/type-detect.js"(exports, module) {
    (function(global3, factory) {
      typeof exports === "object" && typeof module !== "undefined" ? module.exports = factory() : typeof define === "function" && define.amd ? define(factory) : (global3 = typeof globalThis !== "undefined" ? globalThis : global3 || self, global3.typeDetect = factory());
    })(exports, function() {
      "use strict";
...
var require_proxify = __commonJS({
  "node_modules/chai/lib/chai/utils/proxify.js"(exports, module) {
    ...
    function stringDistanceCapped(strA, strB, cap) {
      ...
      var memo = [];
      for (var i3 = 0; i3 <= strA.length; i3++) {
      ...
      for (var i3 = 1; i3 <= strA.length; i3++) {
        var ch = strA.charCodeAt(i3 - 1);
        for (var j = 1; j <= strB.length; j++) {
          if (Math.abs(i3 - j) >= cap) {
            memo[i3][j] = cap;
            continue;
          }
          memo[i3][j] = Math.min(
            memo[i3 - 1][j] + 1,
            memo[i3][j - 1] + 1,
            memo[i3 - 1][j - 1] + (ch === strB.charCodeAt(j - 1) ? 0 : 1)
          );
        }
      }
...
function format(...args) {
  ...
  let str = String(template).replace(formatRegExp, (x) => {
    ...
    switch (x) {
      ...
      case "%o":
        return inspect2(args[i3++], { showHidden: true, showProxy: true });
      case "%O":
  ...
  for (let x = args[i3]; i3 < len; x = args[++i3]) {
...
function serializeError(val, seen = /* @__PURE__ */ new WeakMap()) {
  ...
  if (Array.isArray(val)) {
    ...
    val.forEach((e, i3) => {
      try {
        clone2[i3] = serializeError(e, seen);
      } catch (err) {
...
function buildBySources(decoded, memos) {
  ...
  for (let i3 = 0; i3 < decoded.length; i3++) {
    ...
    for (let j = 0; j < line.length; j++) {
      ...
      insert(originalLine, memo.lastIndex = index + 1, [sourceColumn, i3, seg[COLUMN]]);
    }
  }
  return sources;
}
function insert(array2, index, value) {
  for (let i3 = array2.length; i3 > index; i3--) {
...
function splitByComma(s) {
  ...
  for (let i3 = 0; i3 < s.length; i3++) {
    if (s[i3] === "{" || s[i3] === "[") {
      stack.push(s[i3] === "{" ? "}" : "]");
    } else if (s[i3] === stack[stack.length - 1]) {
      stack.pop();
    } else if (!stack.length && s[i3] === ",") {
      const token = s.substring(start, i3).trim();
      if (token)
        result.push(token);
      start = i3 + 1;
    }
...
function createSuiteCollector(name, factory = () => {
  ...
  async function collect(file) {
    ...
    const allChildren = [];
    for (const i3 of [...factoryQueue, ...tasks])
  ...
  suiteFn.each = function(cases, ...args) {
    ...
    return (name, optionsOrFn, fnOrOptions) => {
      ...
      cases.forEach((i3, idx) => {
        ...
        if (fnFirst) {
          arrayOnlyCases ? suite2(formatTitle(_name, items, idx), () => handler(...items), options) : suite2(formatTitle(_name, items, idx), () => handler(i3), options);
        } else {
          arrayOnlyCases ? suite2(formatTitle(_name, items, idx), options, () => handler(...items)) : suite2(formatTitle(_name, items, idx), options, () => handler(i3));
        }
      });
  ...
  taskFn.each = function(cases, ...args) {
    ...
    return (name, optionsOrFn, fnOrOptions) => {
      ...
      cases.forEach((i3, idx) => {
        const items = Array.isArray(i3) ? i3 : [i3];
        if (fnFirst) {
          arrayOnlyCases ? test22(formatTitle(_name, items, idx), () => handler(...items), options) : test22(formatTitle(_name, items, idx), () => handler(i3), options);
        } else {
          arrayOnlyCases ? test22(formatTitle(_name, items, idx), options, () => handler(...items)) : test22(formatTitle(_name, items, idx), options, () => handler(i3));
        }
      });
      this.setContext("each", void 0);
    };
...
function formatTemplateString(cases, args) {
  ...
  for (let i3 = 0; i3 < Math.floor(args.length / header.length); i3++) {
    ...
    for (let j = 0; j < header.length; j++)
      oneCase[header[j]] = args[i3 * header.length + j];
    res.push(oneCase);
  }
  return res;
}
...
var M2 = (e) => (f2(e, c2, { value: { reset: () => C2(e[c2]) } }), e[c2]);
var A2 = (e) => e[c2] || M2(e);
function I2(e) {
  ...
  let t2 = function(...s) {
    ...
    if (r2.impl)
      try {
        new.target ? p2 = Reflect.construct(r2.impl, s, new.target) : p2 = r2.impl.apply(this, s), d2 = "ok";
      } catch (l2) {
        throw p2 = l2, d2 = "error", r2.results.push([d2, l2]), l2;
      }
    let a2 = [d2, p2];
    if (b2(p2)) {
      let l2 = p2.then((o2) => a2[1] = o2).catch((o2) => {
        throw a2[0] = "error", a2[1] = o2, o2;
      });
      Object.assign(l2, p2), p2 = l2;
    }
    return r2.results.push(a2), p2;
  };
  i2(t2, "_isMockFunction", true), i2(t2, "length", e ? e.length : 0), i2(t2, "name", e && e.name || "spy");
  let n2 = A2(t2);
...
function E(e, t2, n2) {
  ...
  })(), m2 = k(e, s), p2 = Object.getPrototypeOf(e), d2 = p2 && k(p2, s), a2 = m2 || d2;
  R2(a2 || s in e, `${String(s)} does not exist`);
  let l2 = false;
  r2 === "value" && a2 && !a2.value && a2.get && (r2 = "get", l2 = true, n2 = a2.get());
  let o2;
  a2 ? o2 = a2[r2] : r2 !== "value" ? o2 = () => e[s] : o2 = e[s], n2 || (n2 = o2);
  let y2 = I2(n2);
  r2 === "value" && P2(y2, o2);
  let O2 = (h) => {
    let { value: G2, ...w } = a2 || {
    ...
    r2 !== "value" && delete w.writable, w[r2] = h, f2(e, s, w);
  }, K2 = () => a2 ? f2(e, s, a2) : O2(o2), T2 = y2[c2];
  return i2(T2, "restore", K2), i2(T2, "getOriginal", () => l2 ? o2() : o2), i2(T2, "willCall", (h) => (T2.impl = h, y2)), O2(l2 ? () => (P2(y2, n2), y2) : y2), g.add(y2), y2;
}
...
function getObjectKeys(object2) {
  return [
    ...
    ...Object.getOwnPropertySymbols(object2).filter(
      (s) => {
        var _a8;
        return (_a8 = Object.getOwnPropertyDescriptor(object2, s)) == null ? void 0 : _a8.enumerable;
      }
    )
  ];
}
  ...
  const getObjectSubsetWithContext = (seenReferences = /* @__PURE__ */ new WeakMap()) => (object22, subset2) => {
    if (Array.isArray(object22)) {
      if (Array.isArray(subset2) && subset2.length === object22.length) {
        return subset2.map(
          (sub, i3) => getObjectSubsetWithContext(seenReferences)(object22[i3], sub)
        );
      }
...
${spy.mock.calls.map((callArg, i3) => {
        let methodCall = c3().bold(`  ${ordinalOf(i3 + 1)} ${spy.getMockName()} call:
...
${spy.mock.results.map((callReturn, i3) => {
      let methodCall = c3().bold(`  ${ordinalOf(i3 + 1)} ${spy.getMockName()} call return:
...
function decode2(mappings) {
  ...
  do {
    ...
    for (let i3 = index; i3 < semi; i3++) {
      let seg;
      i3 = decodeInteger2(mappings, i3, state, 0);
      const col = state[0];
      if (col < lastCol)
        sorted = false;
      lastCol = col;
      if (hasMoreVlq2(mappings, i3, semi)) {
        i3 = decodeInteger2(mappings, i3, state, 1);
        i3 = decodeInteger2(mappings, i3, state, 2);
        i3 = decodeInteger2(mappings, i3, state, 3);
        if (hasMoreVlq2(mappings, i3, semi)) {
          i3 = decodeInteger2(mappings, i3, state, 4);
          seg = [col, state[1], state[2], state[3], state[4]];
        } else {
          seg = [col, state[1], state[2], state[3]];
        }
...
function hasMoreVlq2(mappings, i3, length) {
  if (i3 >= length)
    return false;
  return mappings.charCodeAt(i3) !== comma2;
}
function sort2(line) {
...
var MockDate = class _MockDate extends RealDate {
  constructor(y2, m2, d2, h, M3, s, ms) {
    super();
    let date;
    switch (arguments.length) {
      ...
      default:
        ...
        s = s || 0;
        ms = ms || 0;
        date = new RealDate(y2, m2, d2, h, M3, s, ms);
        break;
    }
